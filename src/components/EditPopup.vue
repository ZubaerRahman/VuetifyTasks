<template>
    <div>
        ...
        >
            <v-card>
                <v-card-title class="headline">Edit project "{{current_project.title}}"</v-card-title>
                <v-card-text>
                    <v-form class="px-3" ref="form">
                        ...
                        <v-btn text color="red" class="mx-0 mt-3" @click="submit()" :loading="loading">Edit Project</v-btn>
                    </v-form>
                </v-card-text>
            </v-card>
        </v-dialog>
...
<script>
    ...
    export default {
        ...
        data: function () {
            return{
                dialog: null,
                title: this.current_project.title,
                content: this.current_project.content,
                user: null,
        ...
        methods:{
            submit(){
                if(this.$refs.form.validate()){
                    ...
                    db.collection('projects').doc(this.current_project.id).update(project)
                        .then(()=>{
                            this.loading = false;
                            this.dialog = false;
                    ...
                    db.collection('users').doc(current_user.uid).get()
                        .then(user => {
                            if (user.exists) {
                                ...
                                console.log("User not found in EditPopup, contact admin");
